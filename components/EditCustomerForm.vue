      ...
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12>
                <v-text-field hide-details prepend-icon="search" single-line v-model="customer"></v-text-field>
              </v-flex>
              <v-flex xs12>
                <v-textarea outline autofocus auto-grow readonly label="Kunde: " placeholder="Kundendaten" :value="`${customer.preName} ${customer.surName} \n${customer.street} ${customer.streetNr} \n${customer.zipCode} ${customer.city}`" :key="i" v-for="(customer, i) in customerData"></v-textarea>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
